...
<script lang="ts">
    ...
    import { datos2, etiquetas2 } from '../lib/stores.svelte.js';
	...
	const escalaY= $derived(scaleLinear()
		.domain([min(datos2), max(datos2)])
		.range([height-margin.bottom, margin.top]));

    // TODO 3: Generador de Línea ($derived)
    // Debe usar d3.line() con las escalas anteriores para crear el "path"
	const pathString = $derived(line().x((dato, indice) => escalaX(indice)).y((dato) => escalaY(dato)).curve(curveMonotoneX)(datos2));
    ...
    <div class="chart-container" bind:clientWidth={containerWidth}>
        ...
        {#each datos2 as d, i}
            ...
            <text
                ...
                fill="#eee"
            >
                {etiquetas2[i % etiquetas2.length]}
            </text>
            </g>
        {/each}
